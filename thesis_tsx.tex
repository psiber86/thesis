...
Contention will clearly be a problem if the simulation only schedules from one LTSF queue
...
~\ref{fig:noThrMig_timeVSthreads_1schq}.
...
Varying schedule queue countdData is limited for static thread migration as the
number of LTSF queues has to be evenly divisible by the number of worker
threads.
It should be noted that the static assignment of worker threads to LTSF 
queues only allows for a number of worker threads that is evenly
divisible by the number of worker threads due to the way in which LP's are
partitioned to LTSF queues.  
