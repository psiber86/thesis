...
\cite{dickman}.  Each LP maintains its own event set as timestamp ordered queue.
...
useful work.  When a worker thread is created, or resumes execution after being suspended by
the manager thread, it attempts to lock the LTSF queue and dequeue the least
time-stamped event.  If the worker thread succesfully retrieved an event, it
executes that event as specified by the simulation model.  It then attempts to
lock the pending event set for the LP associated with the executed event, and
dequeues the next least timestamped event if it successfully locked the pending
event set queue.  The dequeued event is inserted into the LTSF queue, which
resorts based on the event time stamps.  An abstract event processing algorithm
is shown in Figure ~\ref{workerThreadAlgorithm} \cite{dickman}.  Note that the
worker threads perform many other functions; the previously described actions
are the only relevant events to this research.  The entire pending event set
implementation can be seen in Figure ~\ref{fig:singleLTSFqueue} \cite{dickman}.
