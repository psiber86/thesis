...
the simulation runtimes are compared for the same LTSF queue and worker thread
configurationsj.  When comparing the LTSF queue configurations, the multiple
LTSF queue configuration runtime is compared with the runtime of the same
...
anticipated results.  Simulation runtime actually slightly increased with
additional LTSF queues.  However, HLE and RTM with 1 retry still reduced
simulation runtime when compared to standard locking mechanisms, HLE more so
than RTM.
...
Simulation runtime decreased slightly increasing the LTSF queue with 4 worker
threads.  Each multiple LTSF configuration reduced simulation runtime by 2-3\%
with HLE and RTM using 1 retry generally having larger reductions than standard
locking mechanisms.  The only exception to this trend is the 4 LSTF queue
configuration with HLE; it reduced simulation runtime slightly less than
...
still reduces runtime by about 18\%, while RTM generally generally reduces
runtime by about 3-4\% when comparing the two to standard locking mechanisms.
...
simulation runtime reduced 2\% to 4\% as the number of LTSF queues was
increased.  Using HLE, the runtime was reduced by the same amount as standard
...
actually reduced the simulation runtime the most.  Runtime reductions increased
...
runtime from 6\% to 9\% as the number of LTSF queues is increased.
...
still only outperforms standard locking mechanisms by about 5\%.  However, it
significantly improves performance with increased LTSF queues.  Execution time
was reduced from 9\% to almost 14\% when using RTM with multiple LTSF queues.
...
threads, but no does not improve performance.
...
configuration using standard locking mechanisms configuration took anywherefrom
70 seconds to 160 seconds.  The simulations that took 160 seconds to execute
experienced significantly higher rollbacks.  When the simulation does appear to
run normally, it executes slightly faster than the strictly static thread
assignment scheme.  The instability of this migration scheme made it infeasible
to obtain good data.
...
configurations, HLE reduces execution time by as much as 23\% and consistenly
reduces execution time by 20\%.   

While HLE is the superior synchronization mechanism, RTM still showed increases
in performance, generally by about 5\%.  It also responded to multiple
...
The various RTM configurations allow RTM to retry if contention is
high at certain times.  RTM might not perform as well as HLE due to the overhead
associated with RTM.  The retry algorithm requires abort statistics to be calculated and
maintained which adds a bit more overhead to RTM.  
