...
\textbf{State:} describes the state of the system.\par
...
geographical locations containing a subset of the total population.  Events in
this simulation represent the arrival or departure of individuals to or from
locations, the progression of a diseased individual within a location, the
diffusion of a disease within a geographical location, etc.  \cite{epidemic}.
To effectively model epidemics, a significant population size and number of
locations needs to be simulated.  It is infeasible to perform this simulation on
a sequential machine.  

\indent 
The need for such large simulations has spurred the shift to
Parallel Discrete Event Simulation (PDES) in which LPs are executed
concurrently.  
...
The worker threads "schedule" a certain simulation object by removing the top
event of 
...
The general event execution loop for the wprker threads is shown below in Figure
~\ref{workerThreadAlgorithm} \cite{dickman}.
