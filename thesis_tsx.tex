...
alleviate contention.  Using HLE, simulation execution time was reduced by 13-14\%
regardless of the number of LTSF queues used.  RTM improved performance using
only 1 retry, but only by about 1-3\%.  Using any more retries resulted in worse
performance.  Using the standard locking mechanisms, simulation execution time reduced
by about 2.5\% increasing the LTSF queue count from 1 to 2.  But using TSX
synchronization, simulation execution time reduced by about 4\% increasing the LTSF
...
alleviate contention as well.  Using HLE, simulation execution time was reduced by
about 20\% regardless of the number of LTSF queues used.  RTM improved
performance using only 1 retry, but only by about 5\%.  Using any more retries
resulted in worse performance for 3 LTSF queues, while execution time remained constant
for the number of retries using one LTSF queue.  Increasing the LTSF queue count
from 1 to 3 reduced simulation execution time by about 2.5\% using the standard locking
mechanisms.  But TSX synchronization reduced simulation execution time by about 4\%
...
configurations, simulation execution time trends increased as the number of LTSF queues
used was increased.  It was noted that these simulations resulted in
significantly higher rollbacks, the most likely cause of the increased execution time.
...
Simulation execution time decreased slightly increasing the LTSF queue with 4 worker
threads.  Each multiple LTSF configuration reduced simulation execution time by 2-3\%
with HLE and RTM using 1 retry generally having larger reductions than standard
locking mechanisms.  The only exception to this trend is the 4 LTSF queue
...
in worse performance for higher LTSF queue counts.  In any configuration, HLE
still reduces execution time by about 18\%, while RTM generally generally reduces
execution time by about 3-4\% when comparing the two to standard locking mechanisms.
...
increased.  Using HLE, the execution time was reduced by the same amount as standard
locking mechanisms with increased LTSF queues; however, HLE still outperforms
the standard locking mechanisms by 20\% to begin with.  Contention was still
reduced, but not because multiple LTSF queues were used.  Using RTM with 9 retries
actually reduced the simulation execution time the most.  execution time reductions increased
from 4\% to 7\% as the number of LTSF queues was increased.  In this
configuration, RTM generally outperformed standard locking mechanisms by about
5\%.
...
Again, HLE shows similar reductions in contention.  However, it still
outperforms standard locking by as much as 23\% using 4 LTSF queues.  While RTM
generally only outperforms standard locking by about 5\%, it reduces simulation
...
reassigning
After the thread has executed 50 events, it is reassigned to its original LTSF
queue and continues to schedule from that queue for the remainder of the
simulation.
...
execution in the allotted time.  For instance, the 6 worker thread, 3 LTSF queue
configuration using standard locking mechanisms configuration took anywhere from
70 seconds to 160 seconds.  The longer running simulations experienced
...
scheme.  The instability of this migration scheme made it infeasible to obtain data.
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-3thread-rtm-9retry}
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-4thread-rtm-9retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 9 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-5thread-rtm-9retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 9 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-6thread-rtm-9retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 9 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-7thread-rtm-9retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 9 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-3thread-rtm-19retry}
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-4thread-rtm-19retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 19 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-5thread-rtm-19retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 19 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-6thread-rtm-19retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 19 Retry
...
%    \centering
%    \graphicspath{ {./figures/} }
%    \includegraphics[width=0.75\textwidth,keepaspectratio]{hugeepidemicsim-CONTmig-timeVSschedQs-msVSst-7thread-rtm-19retry}
%\caption{\textbf{Multi-Set VS Splay Tree LTSF Queues for RTM with 19 Retry
%     using 7 Worker Threads}}\label{fig:contThrMig_timeVSschq_7threads_msVSst_rtm19}
%\end{figure}

It is evident that using the splay tree implementation with RTM decreased
execution time slightly more than using the splay tree implementation with standard
locking mechanisms.  While the difference is small, this implies that the data
structure accessed within critical sections is a factor in the overall
effectiveness of transactional memory.  The discrepancy between the HLE and RTM
observations can again be attributed to the fact that RTM has a retry algorithm
that responds intelligently to contention.  Whereas HLE is more likely to abort
at times of high contention, RTM can use abort information to decide when to
...
synchronization.  When synchronized with TSX, specifically RTM, the splay tree
implementation seemed to reduce execution time a bit more.  The way in which
data structures are accessed is a factor in how effectively transactional memory
operates. 
