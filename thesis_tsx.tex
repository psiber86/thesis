...
%% transaction operations or any synchronization.  That's the only way to ensure consistent data/results.
%% And if it's never written to, then it doesn't need to be a critical
%% section/transactional region in the first place.  I would imagine the commit
%% overhead is substantially less when only read operations are performed, but
%% Intel didn't provide that level of implementation detail.
...
%% I put it after the description
...
The entire pending event set implementation can be seen in
Figure\ref{fig:singleLTSFqueue} \cite{dickman}.
