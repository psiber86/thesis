...
queue, based on a circularly incremented counter.  The worker threads are
dynamically reassigned to LTSF queues. The number of LTSF queues is specified in
a configuration file, and has no restrictions as in the static assignment.
\par
...
In the original WARPED implementation, threads are statically assigned to an
LTSF queue.  Contention will clearly be a problem is the simulation only
schedules from one LTSF queue as every worker thread is assigned to that queue.
...
Data is limited for static thread migration as the number of LTSF queues has to
be evenly divisible by the number of worker threads.
...
\subsection{\textbf{Dynamic Thread Assignment}}
...
\subsubsection{\textbf{Event Limited Thread Migration}}
