...
%% JOSH: OK. so i have a qustion.  if one HLE thread fails and falls back to locks, does 
%% everyone else as well??  you need to make this discussion more clear.
...
%% JOSH: OK, this really does not help the reader.  i do not know how to read the example,
%% what is &lock, your statement that ``GCC 4.8 and above....'' does not explain anything
%% to the reader.  what are these items?  besides not knowing what they are, i also do not
%% know why you or them together (__ATOMIC_HLE_ACQUIRE with __ATOMIC_AQUIRE), also what is
%% __atomic_exchange_n and why is it needed.  where does the while loop end?  why is it
%% necessary?  the reader is quite lost.  i assume your description of RTM is equally
%% vague.  please go through this stuff much more carefully.  it might be helpful to have
%% a common example that you present that illustrates a traditional locking
%% implementation, an HLE illustration and an RTM illustration.  
...
thread releases the lock.  The combination of memory models allows for the
locking instructions to be executed with or without elision.  A general
implementation for the HLE software interface is shown in Figure
~\ref{fig:hle_interface}. 
...
various configurations may not reduce contention as much as hoped, HLE
consistenly reduced simulation runtime, as much as 20\% in some cases.  RTM did
not fare quite as well.  More often than not, the simulation performance using RTM was
comparable to the standard synchronization mechansism.  RTM required more
overhead to maintain statistics for the retry algorithm.  While the retry
algorithm meant well, it did not improve the performance of RTM as hoped.  
