...
functional implementations: 1) using the STL multiset data structure, and 2)
using a splay tree data structure.

\setcounter{secnumdepth}{5}
\setcounter{tocdepth}{5}
\paragraph{\textbf{STL MultiSet}}
...
\subsection{\textbf{Previous Contention Solutions}}
...
\subsection{\textbf{Another Contention Solution: Thread Migration}}

\newpage
\section{\textbf{TSX WARPED}}
...
Functions are only explained in terms of the operations pertaining to the
specific data structure they operate on for the sake of clarity.  
\par

\subsection{\textbf{LTSF Queue}}

The following functions require synchronization to access the LTSF
queue:\par
\texttt{insert()} - insert an event into the LTSF queue if an event was inserted
at the beginning of a specific LP's unprocessed queue.\par
\texttt{updatedScheduleQueueAfterExecute()} - inserts the dequeued event from a
specific LP's unprocessed queue into the LTSF queue.\par
\texttt{nextEventToBeScheduledTime()} - returns the time of the event at the
beginning of the LTSF queue.\par
\texttt{clearScheduleQueue()} - clears the LTSF queue.\par
\texttt{setLowestObjectPosition()} - \textbf{not clear on this function}
\texttt{peek()} - peek the lowest event in the LTSF queue.\par
%not consistent with code

\subsection{\textbf{Unprocessed Queue}}

The following functions require synchronization to access a specific LPs
unprocessed queue:\par
\texttt{insert()} - insert an event into a specific LP's unprocessed
queue.\par
\texttt{updatedScheduleQueueAfterExecute()} - dequeue the next least
timestamped event from a specific LP's unprocessed queue.\par
\texttt{getEvent()} - dequeue and return the least timestamped event in the
unprocessed queue; insert event into processed queue.\par
\texttt{getEventIfStraggler()} - same as \texttt{getEvent()} but does not insert
the event into the processed queue as the getEvent function above does.\par
%need to verify
\texttt{peekEvent()} - return a reference to the next event in the LP's
unprocessed queue.\par
\texttt{peekEventCoastForward()} - same as \texttt{peekEvent()}.\par
\texttt{handleAntiMessage()} - delete an event in a specific LP's unprocessed queue
for which the LP received an anti-message.\par 
\texttt{ofcPurge()} - removes all events from the unprocessed queue; used for
optimistic fossil collection, which is beyond the scope of this study.\par
\texttt{peekEventLockUnprocessed()} - peek the first event of a specif LP's
unprocessed queue, but leave the queue locked.\par
\texttt{getMinEventTime()} - get the timestamp of the first event in a specific
LP's unprocessed queue.\par

\subsection{\textbf{Processed Queue}}

The following functions require synchronization to access a specific LPs
processed queue:\par
\texttt{getEvent()} - insert the dequeued event from a specific LP's unprocessed
queue into that LP's processed queue.\par
\texttt{getEventWhileRollback()} - same as getEvent(), except the unprocessed
queue is already locked.\par %need to verify
\texttt{rollback()} - traverse a specifi LP's entire processed queue and remove
any events with a timestamp greater than or equal to the rollback time; the
removed events are placed in the LP's unprocessed queue.\par 
\texttt{fossilCollect()} - remove events satisfying a certain criteria from a
specific LP's processed queue.\par
\texttt{ofcPurge()} - same as the ofcPurge() function for the unprocessed
queue.\par 
...
locations reuiring a total of 119 LPs.  The epidemic is modeled by reaction processes
