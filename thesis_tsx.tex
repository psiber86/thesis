...
~\ref{workerThredAlgorithm}.  In this implementation, the worker thread inserts
the next event into a different LTSF queue, based on a circularly incrementing
counter.
...
\end{verbatim}
\linespread{2.0}
\caption{Generalized event execution loop for migrating worker threads.  Many details
...
architectures.  Jingjing Wang et al. noticed similar performance degredation,
...
section.  If a critical section alwasy writes to the entire shared data
...
timestamped event from a specific LP's unprocessed queue.
\item\texttt{getEvent()} - dequeue and return the least timestamped event in the
...
\item\texttt{getMinEventTime()} - get the timestamp of the first event in a specific
...
queue is already locked.%need to verify
...
interefere can execute concurrently.  TSX tracks read and write memory
operations separately in the transaction's read-set and write-set resepctively.
...
the timestamp of the event at the head of the LTSF queue.  There
...
necessitates sycnhronization.  However, instead of the second worker thread
...
too many possible scenarious to identify specifically where TSX can be
...
work 100\% of the time; there are several runtime events that can cause
transcations to abort, as well as physical limitations.  
\par
...
%is why WARPED can greatly benefit from TSX enabled synchronization.
